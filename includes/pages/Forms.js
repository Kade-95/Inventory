class Forms {
    ...
    view(container) {
        ...
        perceptor.runParallel(fetch, result => {
            ...
            let renderTable = value => {
                let contents = [];
                let selectedForm = perceptor.array.find(result.customForms, form => {
                ...
                for (let form of result.forms) {
                    if (form.type == selectedForm._id) {
                        let formContent = {};
                        perceptor.object.copy(form, formContent);
                ...
                let formsTable = perceptor.createTable({ title: value + ' Forms Table', contents, search: true, sort: true, filter: ['All', 'Enough', 'Excess', 'Low'] });
                container.render(formsTable);

                perceptor.listenTable({ options: ['edit', 'clone', 'delete'], table: formsTable }, {
                    click: event => {
        ...
        system.get({ collection: 'forms', query: { _id: id }, changeQuery: { _id: 'objectid' } }).then(result => {
            ...
            system.get({ collection: 'customforms', query: { _id: type }, changeQuery: { _id: 'objectid' } }).then(selectedForm => {
                let displayForm = (contents, tasks) => {
                        ...
                        buttons: {
                            ...
                            actions: {
                                element: 'div', attributes: { id: 'single-form-actions-container' }, children: [
                                    { element: 'i', attributes: { class: 'action', id: 'add-new-task', title: 'Add New Task', 'data-icon': perceptor.icons.plus } },
                                ]
                            },
                    ...
                    let target = perceptor.inBetween(selectedForm.target, '$#&{', '}&#$');
                    try {
    ...
    create(container) {
        perceptor.runParallel({
            ...
            let types = perceptor.object.valueOfObjectArray(customForms, 'name');
            let selectCell = perceptor.cell({ element: 'select', name: 'Form', dataAttributes: {}, options: types });

            document.body.find('#more-forms-controls').append(selectCell);

            let selectedForm = perceptor.array.find(customForms, form => {
            ...
            this.renderForm(selectedForm, displayForm);
            selectCell.find('#Form-cell').onChanged(value => {
                selectedForm = perceptor.array.find(customForms, form => {
                    return form.name == value;
                });
                this.renderForm(selectedForm, displayForm);
            });
